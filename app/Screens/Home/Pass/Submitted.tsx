...
export const Submitted = (props) => {
  const { open } = useModal();
  const { mutate: returnMutate } = useMyMutation<null, null>("delete", "classroom", "/return");
  ...
  if (props.type === "APPLICATION")
    return (
      <View style={styles.container}>
        ...
        <Button style={{ width: 120 }} textProps={{ fontLevel: 2 }} onPress={() => open(<Out />)}>
          외출증 보기
        </Button>
      </View>
    );
