...
const HelperLink = ({ text, label, onPress, right = false }) => {
  return (
    <View style={{ marginTop: -30, width: "100%", display: "flex", flexDirection: "row", gap: 5, justifyContent: right ? "flex-end" : "flex-start" }}>
      <Text colorLevel={900} colorType="gray" fontType="body" fontLevel={1}>
        {text}
      </Text>
      <Text onPress={onPress} colorLevel={500} colorType="main" fontType="body" fontLevel={1} style={{ textDecorationLine: "underline" }}>
        {label}
      </Text>
    </View>
  );
  ...
  return (
    <KeyboardDismiss>
      <Layout style={{ gap: 40 }}>
        ...
        <HelperLink text={"비밀번호를 잊어버리셨나요?"} label={"비밀번호 변경"} onPress={() => navigation.push("비번변경이메일")} right />

        <View style={{ width: "100%", position: "absolute", bottom: 30, gap: 12 }}>
          <HelperLink text={"PiCK 계정이 없으신가요?"} label={"회원가입"} onPress={() => navigation.push("회원가입이메일")} />
          <Button disabled={!!!data.account_id || !!!data.password} onPress={() => handlePress()}>
            로그인
          </Button>
        </View>
      </Layout>
