...
export const Password = ({ navigation }) => {
  ...
  return (
    <KeyboardDismiss>
      <Layout style={{ gap: 40 }} Header={<PrevHedaer title="비밀번호 변경" />}>
        ...
        <TextInput label="새로운 비밀번호" value={data.password} id="password" placeholder="비밀번호를 입력하세요" onChange={handleChange} password />
        <TextInput label="새로운 비밀번호 확인" value={data.check} id="check" placeholder="비밀번호를 입력하세요" onChange={handleChange} password />
        {isError ? (
